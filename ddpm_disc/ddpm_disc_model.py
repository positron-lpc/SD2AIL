...
class DDPM_Disc(torch.nn.Module):
    ...
    def disc_reward(self, x, simple=False, threshold=0.85):
        batch = x.to(ptu.device)
        disc_cost = self.diffusion.calc_reward(batch)
        if simple:
            diff_sample = self.diffusion.sample(shape=x.shape)
            simple_cost = self.diffusion.calc_reward(diff_sample)
            simple = diff_sample[simple_cost > threshold]
            return disc_cost, simple
        else:
            return disc_cost
